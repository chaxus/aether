...
export const UsageView = ({ type }: { type: 'electricity' | 'gas' | 'water' }) => {
  ...
  return (
    <div className="md:max-w-[452px] max-w-[calc(100dvw-80px)] w-full pb-6 flex flex-col gap-4">
      <motion.div className="flex flex-col" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}>
      ...
      <div className="flex flex-row gap-6 justify-between">
        ...
        <div className="flex flex-row justify-between w-full">
          {usages.map((usage, index) => (
            <div key={usage.day} className="text-sm h-[150px] flex flex-col items-center gap-1 relative">
              <motion.div
                key={`total-${usage.day}`}
                className={`w-2 bg-${color}-${type === 'electricity' ? 100 : 500} rounded-md mt-auto`}
                initial={{ height: 0 }}
              ...
              <motion.div className="text-xs text-zinc-500" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                {usage.day}
              </motion.div>
            </div>
          ))}
